...
def save_summary_plot(data: dict, png_path: Path) -> None:
    ...
    axes[0, 0].plot(x_r, reward.mean(axis=0), color="tab:blue", label="mean reward")
    ...
    axes[1, 0].plot(x_c, cumulative.mean(axis=0), color="tab:green", label="mean cumulative")
...
def main() -> None:
    ...
    print("Running C-SIMPLE MEMORY RESET arm...")
    print(
        f" - max_steps={MAX_STEPS}, episodes={NUM_EPISODES}, seed={SEED}, noise_end={NOISE_END}, "
        f"reset_threshold={RESET_THRESHOLD}, reset_cooldown={RESET_COOLDOWN}"
    )
    ...
    data = rollout_lifetime(
        ...
        memory_reset_cooldown=RESET_COOLDOWN,
        memory_reset_match_threshold=RESET_THRESHOLD,
    ...
    h5_path = OUT_DIR / f"c-simple-1000-reset-{ts}.h5"
    png_path = OUT_DIR / f"c-simple-1000-reset-{ts}.png"
    summary_path = OUT_DIR / f"c-simple-1000-reset-summary-{ts}.json"
